...
class StreamTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _basetest_open_connection(self, open_connection_fut):
    ...
    async def test_invalid_limit(self):
        with self.assertRaisesRegex(ValueError, 'imit'):
            asyncio.StreamReader(limit=0, loop=asyncio.get_running_loop())

        with self.assertRaisesRegex(ValueError, 'imit'):
            asyncio.StreamReader(limit=-1, loop=asyncio.get_running_loop())

    async def test_read_limit(self):
        stream = asyncio.StreamReader(limit=3, loop=asyncio.get_running_loop())
    ...
    async def test_readline_limit(self):
        ...
        stream = asyncio.StreamReader(limit=7, loop=asyncio.get_running_loop())
        def cb():
            stream.feed_data(b'chunk1')
            stream.feed_data(b'chunk2')
            stream.feed_data(b'chunk3\n')
    ...
    async def test_readuntil_limit_found_sep(self):
        stream = asyncio.StreamReader(loop=asyncio.get_running_loop(), limit=3)
    ...
    async def test_exception_cancel(self):
        stream = asyncio.StreamReader(loop=asyncio.get_running_loop())

        async def loop_emptiness_token():
            pass

        t = asyncio.get_running_loop().create_task(stream.readline())
        await loop_emptiness_token()
        t.cancel()
        await loop_emptiness_token()
...
"""
        ...
        pipe = open(rfd, 'rb', 0)
        reader = asyncio.StreamReader(loop=asyncio.get_running_loop(), limit=1)
    ...
    async def test_streamreader_constructor_use_running_loop(self):
        ...
        self.assertIs(reader._loop, asyncio.get_running_loop())

    async def test_streamreader_constructor_use_global_loop(self):
        # asyncio issue #184: Ensure that StreamReaderProtocol constructor
        # retrieves the current loop if the loop parameter is not set
        # Deprecated in 3.10
    ...
    async def test_streamreaderprotocol_constructor_use_running_loop(self):
        ...
        self.assertIs(protocol._loop, asyncio.get_running_loop())

    async def test_streamreaderprotocol_constructor_use_global_loop(self):
        # asyncio issue #184: Ensure that StreamReaderProtocol constructor
        # retrieves the current loop if the loop parameter is not set
        # Deprecated in 3.10
        self.addCleanup(asyncio.set_event_loop, None)
...
class ReprTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test___repr__exception(self):
        stream = asyncio.StreamReader(loop=asyncio.get_running_loop())
        exc = RuntimeError()
        stream.set_exception(exc)
    ...
    async def test___repr__waiter(self):
        stream = asyncio.StreamReader(loop=asyncio.get_running_loop())
        stream._waiter = asyncio.Future(loop=asyncio.get_running_loop())
        self.assertRegex(
            repr(stream),
    ...
    async def test___repr__transport(self):
        stream = asyncio.StreamReader(loop=asyncio.get_running_loop())
